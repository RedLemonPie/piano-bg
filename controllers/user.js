    ...
    static async create(ctx) {
        ...
        if (user.username && user.password) {
            ...
            if (existUser) {
                ...
                ctx.body = statusCode.ERROR_403('手机号已被注册')
            } else {
                ...
                // 储存token失效有效期1小时
                const token = jwt.sign(userToken, secret.sign, {expiresIn: '1d'});
    ...
    static async getUserInfo(ctx) {
        ...
        if (token) {
            ...
            try {
                ...
                const newUser = await userModel.findUserByName( payload.username)
                ctx.response.status = 200;
                ctx.body = statusCode.SUCCESS_200('查询成功', newUser)
            } catch (err) {
